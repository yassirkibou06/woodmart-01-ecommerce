...
const CardOne = ({item}) => {
    ...
    return (
        <motion.div
            onHoverStart={() => setHoverCardId(item.id)}
            onMouseLeave={() => setHoverCardId(null)}
            key={item.id}
            className={`mt-8 py-2 bg-white w-[220px] lg:w-[240px] h-[480px] rounded-lg relative`} >
            <motion.div
                initial={{ y: 0 }}
                whileHover={{ y: -10, boxShadow: "0px 0px 10px rgba(0, 0, 0, 0.3)" }}
                transition={{ duration: 0.5, type: "spring" }}
                className={`container p-2 absolute top-0 bg-white h-fit rounded-lg ${hoverCardId === item.id ? "z-[999]" : "z-30"}`}>
                <div style={{ pointerEvents: "none" }} className="bg-gray-200 rounded-lg w-full h-[240px] relative z-[2]">
                    <img
                        src={item.images[imageIndex[item.id] || 0]}
                        alt={item.brand}
                        className={` w-full h-full object-cover rounded-lg`} />
                </div>
                <div style={{ pointerEvents: "auto" }} className={`grid grid-cols-3 w-full h-[256px] gap-1 p-2 left-0 top-1 absolute z-[1] ${hoverCardId === item.id ? "visible" : "invisible"}`}>
                ...
                <div className="pt-2 flex flex-col space-y-2">
                    ...
                    <div className="flex items-center mt-3">{item.rating === "5.0" ? <>
                    ...
                    <div className="flex items-center space-x-1"><BiCheck className="text-primary text-2xl" /> <p className="font-medium text-sm lg:text-base">In stock</p></div>
                    <p className="text-primary text-base lg:text-sm font-semibold"><span className="text-gray-300 font-normal text-sm line-through mr-1">{item.oldPrice}</span>{item.price}</p>
                    {/**button "add" start */}
                    <motion.a
                        initial={false}
                        ...
                        className="overflow-hidden cursor-pointer bg-primary px-3 h-[44px] rounded-md flex flex-col items-center justify-center text-white hover:bg-primary/90">
                ...
                <motion.div
                    animate={hoverCardId === item.id ? "visible" : "hidden"}
                    variants={{
